...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        // Check if the same combination of title and description exists in the database
        $sqlCheck = "SELECT COUNT(*) AS count FROM notification WHERE title=:title AND description=:description";
        $queryCheck = $dbh->prepare($sqlCheck);
        $queryCheck->bindParam(':title', $title, PDO::PARAM_STR);
        $queryCheck->bindParam(':description', $description, PDO::PARAM_STR);
        $queryCheck->execute();
                    ...
                    <div class="panel-body">
                      <form method="post" class="form-horizontal">
                        ...
                        <div class="form-group">
                          <label class="col-sm-1 control-label">Topic
                          </label>
                          <div class="col-sm-3">
                            <input type="text" class="form-control" name="title" required>
                          </div>
                          <label class="col-sm-1 control-label">Information
                          </label>
                          <div class="col-sm-5">
                            <input type="text" class="form-control" name="desc" required>
                          </div>
                          <div class="col-sm-2">
                            <button class="btn btn-primary" name="submit" type="submit">POST
                            </button>
                          </div>
                        </div>
                      </form>
